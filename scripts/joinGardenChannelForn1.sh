...
function joinChannel() {
  ...
  set -x
    peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.bacheca.com -c gardenchannel -f ./channel-artifacts/GardenForn1Anchors.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log.txt
    res=$?
    set +x
  cat log.txt
